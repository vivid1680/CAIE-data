    ...
    for page_num in range(len(doc)):
        ...
        for i, block in enumerate(text_blocks):
            ...
            if re.match(r"Fig\.\s*(\d+)\.(\d+)", text) and len(text) < 100:
                ...
                # Logic to find the top of the context (question text)
                top_y = 0
                for j in range(i-1, -1, -1):
                    b_text = text_blocks[j][4].strip()
                    b_rect = fitz.Rect(text_blocks[j][:4])
                    
                    # Target question text phrases or numbers
                    if (re.match(r"^\(\w\)", b_text) or # (a), (b(i))
                        re.match(r"^\d+\s+[A-Z]", b_text) or # 1 A ...
                        "is shown in Fig." in b_text or
...
if __name__ == "__main__":
    extract_complete_figures("9702_s24_qp_22.pdf", "s24_22")
